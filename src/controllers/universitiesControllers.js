...
class UniversitiesControllers {
  async getUniversities(req, res) {
    ...
    try {
      let result;
      if (country) result = await getUniversities.byCountry(req);
      else result = await getUniversities.allUniversities();
  ...
  async getUniversitiesById(req, res) {
    ...
    try {
      ...
      return res.status(500).send({ error: 'Id inexistente no banco de dados' });
    }
  }
  ...
  async putUniversity(req, res) {
    const id = req.params.id;
    const body = req.body;
    if (!body || !id) return res.status('404').send({ error: 'Não possui body ou id.' });
